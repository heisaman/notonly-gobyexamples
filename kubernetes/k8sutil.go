...
func CreateOrUpdateService(ctx context.Context, sclient clientv1.ServiceInterface, svc *v1.Service) error {
	level.Info(logger).Log("msg", "Create or update service...")
	...
	if apierrors.IsNotFound(err) {
		level.Info(logger).Log("msg", "Service not found")
		_, err = sclient.Create(ctx, svc, metav1.CreateOptions{})
		level.Debug(logger).Log("msg", "Create service", "err", err)
		if err != nil {
...
func CreateOrUpdateEndpoints(ctx context.Context, eclient clientv1.EndpointsInterface, eps *v1.Endpoints) error {
	level.Info(logger).Log("msg", "Create or update endpoints...")
	...
	if apierrors.IsNotFound(err) {
		level.Info(logger).Log("msg", "Endpoints not found")
		_, err = eclient.Create(ctx, eps, metav1.CreateOptions{})
		level.Debug(logger).Log("msg", "Create endpoints", "err", err)
		if err != nil {
